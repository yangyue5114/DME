...
import torch
import sys
sys.path.append("/mnt/petrelfs/yangyue/dynamic_eval/evaluation/inpainting/PowerPaint")
...
def predict(input_image, prompt, fitting_degree, ddim_steps, scale, seed,
    ...
    # 调整图像大小，640/512
    if task!='image-outpainting' and (size1>1000 or size2>1000):
        if size1 < size2:
...
parser.add_argument('--task', type=str, default='object-removal', help="['text-guided', 'object-removal', 'shape-guided', 'image-outpainting']", required=True)
parser.add_argument('--vertical_expansion_ratio', type=float, default=1, help='Prompt to guide.')
parser.add_argument('--horizontal_expansion_ratio', type=float, default=1, help='Prompt to guide.')

parser.add_argument('--mask_json_path', type=str, default='/mnt/petrelfs/yangyue/SoM/som_remove/new/SEEDBench/image_to_be_remove.json', required=True)
parser.add_argument('--result_save_dir', type=str, default='/mnt/petrelfs/yangyue/SoM/som_remove/new/remove_task_results_7_11/SEEDBench', required=True)
args = parser.parse_args()



# main_mme_tasks_dir = '/mnt/petrelfs/yangyue/dynamic_eval/evaluation/generate_mask/ten_masks/SEED_Bench/remove'
# result_save_dir = '/mnt/petrelfs/yangyue/dynamic_eval/evaluation/inpainting/PowerPaint/remove_task_results/SEED_Bench'
mask_json_path = args.mask_json_path
result_save_dir = args.result_save_dir
os.makedirs(result_save_dir, exist_ok=True)


# 打开文件并读取数据
with open(mask_json_path, 'r') as file:
    # 加载 JSON 数据
    mask_json_datas = json.load(file)

new_mask_json_datas = []
success_num = 0

for image_mask_json_data in mask_json_datas:
    
    if "index_path" in image_mask_json_data:
            per_original_path = image_mask_json_data["index_path"]
    else:
        per_original_path = image_mask_json_data["img_path"]
    
    if "mask_info_list" in image_mask_json_data:
        image_mask_info_list = image_mask_json_data["mask_info_list"]
        
        for per_mask in image_mask_info_list:
            per_mask_path = per_mask["per_mask_path"]
            per_mask_name = per_mask["per_mask_name"]
            ...
            # ---remove
            image_path = per_original_path
            clip_source_img = image_path  #计算clipscore时记录的json
            if per_mask_name!="unknown":
                text_guided_prompt = per_mask_name
            else:
                text_guided_prompt = ""
            # save_text_guided_prompt = img_mask_data['object_name']
            # text_guided_prompt = ""
            mask_path = per_mask_path
            ...
            original_dir_name = os.path.dirname(per_mask_path)
            inpainted_save_dir_name = original_dir_name.replace("/mnt/petrelfs/yangyue/SoM/som_remove/new", "/mnt/petrelfs/yangyue/SoM/som_remove/new/remove_task_results_7_11")
            # print(inpainted_save_dir_name)
            # assert 1==2
            os.makedirs(inpainted_save_dir_name, exist_ok=True)
            
            original_base_name = os.path.basename(per_mask_path).replace(".jpg","") + "_" + per_mask_name.replace(" ","_")
            inpainted_save_dir_name = os.path.join(inpainted_save_dir_name, original_base_name)
            # print(inpainted_save_dir_name)
            # assert 1==2
            os.makedirs(inpainted_save_dir_name, exist_ok=True)
            
            
            # tast_result_save_dir = os.path.join(result_save_dir, filename)
            # os.makedirs(tast_result_save_dir, exist_ok=True)

            # per_img_result_dir = make_result_dir_per_img(image_path,tast_result_save_dir)
            
            # img_result_save_path_name = os.path.join(per_img_result_dir,save_text_guided_prompt.replace(" ", "_"))
            # os.makedirs(img_result_save_path_name, exist_ok=True)

            # now_T3 = time.strftime('%m%d-%H-%M-%S', time.localtime())
            for index, item in enumerate(inpaint_result):
                save_inpaint_jpg = f'{inpainted_save_dir_name}/inpaint_result_{index}.jpg'
                item.save(save_inpaint_jpg, 'JPEG')
                if index==1:
                    clip_pair={'clip_source_img':clip_source_img,'clip_result_img':save_inpaint_jpg}
                    per_mask["per_after_remove_image_path"] = save_inpaint_jpg
                    # clip_infos.append(clip_pair)

            for index, item in enumerate(gallery):
                save_gallery_jpg = f'{inpainted_save_dir_name}/gallery_result_{index}.jpg'
                item.save(save_gallery_jpg, 'JPEG')
        
    
        print(image_mask_json_data)
        # assert 1==2
        new_mask_json_datas.append(image_mask_json_data)
...
with open(f"{result_save_dir}/image_after_remove.json", "w") as f:
    json.dump(new_mask_json_datas, f, indent=4, ensure_ascii=False, separators=(',', ': '))

print("success_num:", success_num)
    

# for filename in os.listdir(main_mme_tasks_dir):
#     if filename!= 'e':
#         per_mme_task_dir = os.path.join(main_mme_tasks_dir, filename)
#         print(per_mme_task_dir)
#         parameter_json_files_paths = find_parameter_json_files(per_mme_task_dir)
#         print(parameter_json_files_paths)

#         #处理每个task
#         for parameter_json_files_path in parameter_json_files_paths:
#             img_mask_json_path = parameter_json_files_path
#             # 打开并读取 JSON 文件
#             with open(img_mask_json_path, 'r') as file:
#                 img_mask_datas = json.load(file)

#             clip_infos = []
#             per_img_result_dir = ""

#             for img_mask_data in img_mask_datas:
#                 image_path = img_mask_data['source_img_path']
#                 clip_source_img = image_path  #计算clipscore时记录的json
#                 # text_guided_prompt = img_mask_data['object_name']
#                 save_text_guided_prompt = img_mask_data['object_name']
#                 text_guided_prompt = ""
#                 mask_path = img_mask_data['mask_img_path']
#                 mask_image = Image.open(mask_path)
#                 image_image = Image.open(image_path)
#                 input_image = {'image':image_image, 'mask':mask_image}
...
#                 # now_T3 = time.strftime('%m%d-%H-%M-%S', time.localtime())
#                 for index, item in enumerate(inpaint_result):
#                     save_inpaint_jpg = f'{img_result_save_path_name}/inpaint_result_{index}.jpg'
#                     item.save(save_inpaint_jpg, 'JPEG')
#                     if index==1:
#                         clip_pair={'clip_source_img':clip_source_img,'clip_result_img':save_inpaint_jpg}
#                         clip_infos.append(clip_pair)
...
#                 for index, item in enumerate(gallery):
#                     save_gallery_jpg = f'{img_result_save_path_name}/gallery_result_{index}.jpg'
#                     item.save(save_gallery_jpg, 'JPEG')


#             with open(f"{per_img_result_dir}/clip_compute_info.json", "w") as f:
